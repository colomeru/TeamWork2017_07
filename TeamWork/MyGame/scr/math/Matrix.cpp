...
Matrix Matrix::CreateWorld(const Vector3 & scale, const Matrix & rotate, const Vector3 & translation)
{
	return CreateScale(scale) * rotate * CreateTranslation(translation);
}
...
Matrix Matrix::CreateWorld(const Vector3 & position, const Vector3 & forward, const Vector3 & up)
{
	...
	return Matrix(Identity).Left(l)
						   .Up(u)
						   .Forward(f)
						   .Translation(position);
}

Matrix Matrix::CreatePerspectiveFieldOfView(float fov, float aspect, float near_, float far_)
{
...
Matrix Matrix::InvertFast(const Matrix & matrix)
{
	...
	Matrix inv_rotation		= Matrix::Transpose(matrix.RotationMatrix());
	// ïΩçsà⁄ìÆê¨ï™ÇÃãtïœä∑
