...
void Chara_Collision(CHARA *ch, VECTOR *ch_MoveVec, CHARA *chk_ch)
{
	...
	// 移動後の ch の座標を算出,ch_MoveVecは今回のフレームの移動量で,ch->Positionは前フレームの位置。Positionの正式な決定はCollision後に行う
		...
		chk_ch->Position, VAdd(chk_ch->Position, VGet(0.0f, CHARA_HIT_HEIGHT, 0.0f)), CHARA_HIT_WIDTH) == TRUE)
	{
		...
		//横でしか当たらないため、カプセルの半径が距離である事を前提として判定を行う(2キャラ共に半径がCHARA_HIT_WIDTHとする)
	...
	// 当たり判定処理後の移動ベクトルをセット(最終的なプレイヤーのPositionの補正結果から、前回のプレイヤーの位置を引く事で、今回のフレームの最終的な移動量を算出する)
